...
void StrategyDoubleMovingAverage::onKLine(KLine kLine)
{
	if (instruments.contains(kLine.InstrumentID)) {
		...
		if (longMA[0] >= shortMA[0] && longMA[1] < shortMA[1]) {
			sell(kLine.InstrumentID, q[0].closePrice, std::max(1, (positionsMap[kLine.InstrumentID].OpenVolume - positionsMap[kLine.InstrumentID].CloseVolume) / 2));
		}
		// 短均线上穿长均线，做多（买）
		if (shortMA[0] >= longMA[0] && shortMA[1] < longMA[1]) {
			buy(kLine.InstrumentID, q[0].closePrice, std::max(1, int(tradingAccount.Available / q[0].closePrice / 2)));
		}
	}
}
